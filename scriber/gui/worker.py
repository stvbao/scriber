...
from scriber.core.batch import BatchConfig
...
class Worker(QObject):
    ...
    def configure(self, config: dict):
        batch_config = BatchConfig.from_mapping(config)
        self._config_json = json.dumps(batch_config.to_mapping(), ensure_ascii=False)
    ...
    def begin(self):
        self._begin_requested = True
        if not self._ready_announced or self._write_closed:
            return
    ...
    def isReady(self) -> bool:
        return self._ready_announced and self.isRunning() and not self._write_closed
    ...
    def _handle_stdout_line(self, line: str):
        ...
        if event_type == "ready":
            ...
            self.ready.emit()
            if self._begin_requested and not self._write_closed:
                self.begin()
        elif event_type == "log":
